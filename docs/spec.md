...
- londonjourneycli tfl trip --from <origin> --to <destination> [journey flags from tfl journey] [--limit N]
...
In JSON mode, trip returns status, message, from, to, selected, journeys, fare, disruptions, and accessibility. It uses TfL Journey Planner fare data when present and checks active disruptions for the selected option's lines. If Journey Planner returns no options, status is "no_journey" and the command exits 5.

...
func TestCLIFormattingHelpers(t *testing.T) {
	...
	if got := defaultAccessibleStationStopTypes([]string{"tube", "national-rail", "river-bus", "bus", "coach"}); strings.Join(got, ",") != "NaptanMetroStation,NaptanRailStation,NaptanFerryPort,NaptanPublicBusCoachTram,NaptanCoachStation" {
		t.Fatalf("accessible stop types=%v", got)
	}
	if got := defaultAccessibleStationStopTypes([]string{"elizabeth-line"}); strings.Join(got, ",") != "NaptanMetroStation,NaptanRailStation" {
		t.Fatalf("elizabeth line stop types=%v", got)
	}
	if got := defaultAccessibleStationStopTypes(nil); len(got) != 5 {
...
func TestTFLCommandUsageEdges(t *testing.T) {
	cases := []struct {
		...
		{name: "nearby missing coordinates", args: []string{"tfl", "nearby-stops"}, code: exitcode.Usage, want: "provide --location or both --lat and --lon"},
		{name: "nearby location with lat", args: []string{"tfl", "nearby-stops", "--location", "51.5,-0.1", "--lat", "51.5"}, code: exitcode.Usage, want: "--location cannot be combined"},
		{name: "nearby bad location", args: []string{"tfl", "nearby-stops", "--location", "somewhere nearby"}, code: exitcode.Usage, want: "could not parse --location"},
		{name: "nearby incidental number pair", args: []string{"tfl", "nearby-stops", "--location", "meet at 10, 20 by the cafe"}, code: exitcode.Usage, want: "could not parse --location"},
		{name: "nearby unrelated URL number pair", args: []string{"tfl", "nearby-stops", "--location", "https://example.com/path/10,20/x"}, code: exitcode.Usage, want: "could not parse --location"},
		{name: "nearby invalid location latitude", args: []string{"tfl", "nearby-stops", "--location", "95,-0.1"}, code: exitcode.Usage, want: "latitude must be between"},
		...
		{name: "arrivals missing stop and query", args: []string{"tfl", "arrivals"}, code: exitcode.Usage, want: "provide exactly one"},
		{name: "arrivals stop and query", args: []string{"tfl", "arrivals", "--stop", "490", "--query", "London"}, code: exitcode.Usage, want: "provide exactly one"},
		{name: "arrivals bad search limit", args: []string{"tfl", "arrivals", "--query", "London", "--search-limit", "0"}, code: exitcode.Usage, want: "--search-limit must be > 0"},
		{name: "arrivals query zero limit", args: []string{"tfl", "arrivals", "--query", "London", "--limit", "0"}, code: exitcode.Usage, want: "--limit must be > 0 with --query"},

...
func TestTFLCommandsWithFakeServer(t *testing.T) {
	...
	mux.HandleFunc("/StopPoint/Search", func(w http.ResponseWriter, r *http.Request) {
		...
		case "Missing Station":
			_, _ = w.Write([]byte("{\"Query\":\"Missing Station\",\"Total\":0,\"Matches\":[]}"))
		default:
	...
	cases := []struct {
		...
		{name: "nearby location pair", args: []string{"--json", "tfl", "nearby-stops", "--location", "51.505,-0.087", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location geo", args: []string{"--json", "tfl", "nearby-stops", "--location", "geo:51.505,-0.087;u=20", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location geo query", args: []string{"--json", "tfl", "nearby-stops", "--location", "geo:0,0?q=51.505,-0.087(London Bridge)", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location openclaw pin text", args: []string{"--json", "tfl", "nearby-stops", "--location", "📍 51.505000, -0.087000", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location openclaw live text", args: []string{"--json", "tfl", "nearby-stops", "--location", "🛰 Live location: 51.505000, -0.087000 ±8m", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location google", args: []string{"--json", "tfl", "nearby-stops", "--location", "https://www.google.com/maps/search/?api=1&query=51.505,-0.087", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		{name: "nearby location openclaw context", args: []string{"--json", "tfl", "nearby-stops", "--location", "LocationLat: 51.505\nLocationLon: -0.087", "--limit", "1"}, want: "London Bridge Bus Station", code: exitcode.OK},
		...
		{name: "zone anytime json envelope failure", args: []string{"--json", "--envelope", "tfl", "fares", "--from-zone", "3", "--to-zone", "1", "--period", "anytime", "--payment", "contactless"}, want: "\"ok\": false", code: exitcode.NoData},

...
func cmdTFL(ctx context.Context, g globals, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: londonjourneycli tfl <status|disruptions|line-routes|nearby-stops|accessible-stations|stop-search|stop-info|arrivals|next-arrival|journey|watch-arrival>")
		return exitcode.Usage
	}
...
func defaultAccessibleStationStopTypes(modes []string) []string {
	if len(modes) == 0 {
		return []string{"NaptanMetroStation", "NaptanRailStation", "NaptanFerryPort", "NaptanBusCoachStation"}
	}
	...
	for _, mode := range modes {
		...
		case "tube", "dlr", "elizabeth-line", "tram":

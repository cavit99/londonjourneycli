...
func cmdTFL(ctx context.Context, g globals, args []string, stdout, stderr io.Writer) int {
	if len(args) == 0 {
		fmt.Fprintln(stderr, "usage: londonjourneycli tfl <status|disruptions|line-routes|nearby-stops|accessible-stations|stop-search|stop-info|arrivals|next-arrival|journey|trip|compare|fare|fares|watch-arrival>")
		return exitcode.Usage
	}
...
func tflTrip(ctx context.Context, g globals, client *tfl.Client, args []string, stdout, stderr io.Writer) int {
	...
	opts := values.options()
	resp, err := client.Journey(ctx, values.From, values.To, opts)
	if err != nil {
		if code, ok := writeStructuredTfLError(g, stdout, stderr, err); ok {
			return code
		}
	...
	result := buildTripResult(values.From, values.To, resp, opts, *limit)
...
func printHelp(w io.Writer) {
	...
	fmt.Fprintln(w, "  tfl trip --from A --to B     Plan with fare, disruptions, and access notes")
